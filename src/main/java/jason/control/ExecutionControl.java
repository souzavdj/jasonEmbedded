...
public class ExecutionControl {
        ...
        new Thread("ExecControlWaitAgFinish") {
            public void run() {
                ...
                try {
                    while (isRunning) {
                        try {
                            boolean to = !agFinishedCond.await(getCycleTimeout(),
                                    TimeUnit.MILLISECONDS); // waits signal
                            if (!isRunning) {
                                break;
                            }
    ...
    public void receiveFinishedCycle(String agName, boolean breakpoint, int cycle) {
        ...
        if (cycle == this.cycleNumber && runningCycle) { // the agent finished the current cycle
            ...
            try {
                if (logger.isLoggable(Level.FINE)) {
                    logger.fine("Agent " + agName + " has finished cycle " + cycle + ", # of finished agents is " + (
                            finished.size() + 1) + "/" + nbAgs);
                    if (breakpoint) {
