...
public class ExecutionControlGUI extends ExecutionControl {
    ...
    void initComponents() {
        ...
        jCbWho.addItem("selected agent.asl");

        jBtRun = new JButton("Run", new ImageIcon(ExecutionControlGUI.class.getResource("/images/run.gif")));
        jBtRun.setToolTipText(
                "Run the MAS until some agent.asl achieve a breakpoint. Breakpoints are annotations in plans' label");
        ...
        jTA.setText("<html><body>Select the agent.asl to inspect.</body></html>");
        ...
        spTA.setBorder(BorderFactory
                .createTitledBorder(BorderFactory.createEtchedBorder(), "Agent Inspection", TitledBorder.LEFT,
                        TitledBorder.TOP));

        JPanel pHistory = new JPanel(new BorderLayout());//new FlowLayout(FlowLayout.CENTER));
        pHistory.setBorder(BorderFactory
                .createTitledBorder(BorderFactory.createEtchedBorder(), "Agent History", TitledBorder.LEFT,
                        TitledBorder.TOP));
        ...
        spList.setBorder(BorderFactory
                .createTitledBorder(BorderFactory.createEtchedBorder(), "Agents", TitledBorder.LEFT, TitledBorder.TOP));
    ...
    private void inspectAgent(String agName) {
        ...
        if (agState == null) {
            try {
                ...
                jTA.setText("Error getting the state of agent.asl " + currentAgName);
    ...
    public void receiveFinishedCycle(final String agName, boolean breakpoint, final int cycle) {
        if (!listModel.contains(agName)) {
            logger.fine("New agent.asl " + agName);
        ...
        try {
            ...
            jTA.setText("Error getting the state of agent.asl " + agName);
            logger.log(Level.SEVERE, "Error:", e);
        }
