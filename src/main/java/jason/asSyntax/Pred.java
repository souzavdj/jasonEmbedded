...
public class Pred extends Structure {
    ...
    */
    ...
    private final boolean applyAnnots(Unifier u) {
        boolean r  = false;
        if (annots != null) {
            // if some annotation has variables that become ground, they need to be replaced in the list to maintain
            the order
            List<Term> toAdd = null;
            Iterator<ListTerm> i = annots.listTermIterator();
            while (i.hasNext()) {
                ListTerm lt = i.next();
                if (lt.isTail() && lt.getTail().apply(u)) { // have to test tail before term, since term test may
                lead to i.remove that remove also the tail
                    r = true;
    ...
    */
    ...
    public boolean hasSubsetAnnot(Literal p, Unifier u) {
        ...
        while (i1.hasNext()) {
            ...
            if (annot.isVar()
                    && !i2Reset) { // when we arrive to the vars in the annots of this, we need to start searching
                // from the begin again
                i2Reset = true;
                i2 = pAnnots.iterator();
                pAnnot = null;
            }
            ...
            while (true) {
                if (pAnnot != null && u.unifiesNoUndo(annot, pAnnot)) {
                    ...
                    break; // quite the loop, the current p annot is greater than this annot, so annot is not in p's
                    // annots
                } else if (i2.hasNext()) {
                    pAnnot = i2.next();
                } else {
    ...
    public boolean equals(Object o) {
        ...
        if (o instanceof Pred) {
            ...
            return super.equals(o) && this.hasSubsetAnnot(p) && p.hasSubsetAnnot(this);
                ...
                && !hasAnnot()) { // if o is some object that extends Atom (e.g. structure), goes to super equals
            return super.equals(o);                     // consider super equals only when this has no annots
        }
        return false;
    }

    public boolean equalsAsStructure(
            Object p) { // this method must be in this class, do not move (I do not remember why!)
