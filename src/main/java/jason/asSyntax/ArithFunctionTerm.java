    ...
    public Term capply(Unifier u) {
        if (function == null) {
            logger.log(Level.SEVERE,
                    getErrorMsg() + " -- the function can not be evalutated, it has no function assigned to it!",
                    new Exception());
        } else {
            Term v = super.capply(u);
            if (function.allowUngroundTerms() || v.isGround()) {
                try {
                    value = new NumberTermImpl(
                            function.evaluate((agent == null ? null : agent.getTS()), ((Literal) v).getTermsArray()));
                    return value;
                ...
                //} else {
                //    logger.warning(getErrorMsg()+ " -- this function has unground arguments and can not be
                // evaluated! Unifier is "+u);
            }
        }
        return clone();
    }
