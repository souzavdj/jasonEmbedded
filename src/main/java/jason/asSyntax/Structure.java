...
public class Structure extends Atom {
    ...
    protected static final Term[] emptyTermArray = new Term[0];
            // just to have a type for toArray in the getTermsArray method
    ...
    public boolean subsumes(Term t) {
        if (t.isStructure()) {
            ...
            final int oa = tAsStruct.getArity();
            for (int i = 0; i < ma && i < oa; i++) {
                //System.out.println(getTerm(i)+" comp "+tAsStruct.getTerm(i)+"="+getTerm(i).isMoreGeneral(tAsStruct
                // .getTerm(i)));
    ...
    */
    ...
    public VarTerm varToReplace(Term t, Unifier un) {
        ...
        if (deref.equals(vt)) {
            // forget the name
            UnnamedVar var = useShortUnnamedVars ? new UnnamedVar() : new UnnamedVar(
                    "_" + UnnamedVar.getUniqueId() + t);
