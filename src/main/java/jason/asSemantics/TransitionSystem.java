...
public class TransitionSystem {
    ...
    public Timer intentionTimer = new Timer();

    // test
    int execStep = 0;

    int cicleNumber = 3000;
    ...
    private String logStatus = new String();
    ...
    private int nrcslbr = Settings.ODefaultNRC; // number of reasoning cycles
    ...
    private Map<GoalListener, CircumstanceListener> listenersMap; // map the
    ...
    /**
     * add default error annotations (error, error_msg, code, code_src,
     * code_line) in the failure event
     */
    ...
    /**
     * adds an object that will be notified about events on goals (creation,
     * suspension, ...)
     */
    ...
    /*
     * private void applyProcMsgCN() throws JasonException { confP.step =
     * State.SelEv; if (this.agArch.getCommBridge().hasMsg()) { Message m =
     * this.agArch.getCommBridge().checkMailCN();
     * this.agArch.getCommBridge().cleanMailBox(); if (m == null) return;
     *
     * // get the content, it can be any term (literal, list, number, ...; //
     * see ask) Term content = null; if (m.getPropCont() instanceof Term) {
     * content = (Term) m.getPropCont(); } else { try { content =
     * ASSyntax.parseTerm(m.getPropCont().toString()); } catch (ParseException
     * e) { logger.warning("The content of the message '" + m.getPropCont() +
     * "' is not a term!"); return; } }
     *
     * // check if an intention was suspended waiting this message Intention
     * intention = null; if (m.getInReplyTo() != null) { intention =
     * getC().getPendingIntentions().get(m.getInReplyTo()); } // is it a pending
     * intention? if (intention != null) { // unify the message answer with the
     * .send fourth argument. // the send that put the intention in Pending
     * state was // something like // .send(ag1,askOne, value, X) // if the
     * answer was tell 3, unifies X=3 // if the answer was untell 3, unifies
     * X=false Structure send = (Structure) intention.peek().getCurrentStep()
     * .getBodyTerm(); if (m.isUnTell() &&
     * send.getTerm(1).toString().equals("askOne")) { content = Literal.LFalse;
     * } else if (content.isLiteral()) { // adds source in the content // if
     * possible content = ((Literal) content).forceFullLiteralImpl(); ((Literal)
     * content).addSource(new Atom(m.getSender())); } else if
     * (send.getTerm(1).toString().equals("askAll") && content.isList()) { //
     * adds source in each answer if // possible ListTerm tail = new
     * ListTermImpl(); for (Term t : ((ListTerm) content)) { if (t.isLiteral())
     * { t = ((Literal) t).forceFullLiteralImpl(); ((Literal) t).addSource(new
     * Atom(m.getSender())); } tail.append(t); } content = tail; }
     *
     * // test the case of sync ask with many receivers Unifier un =
     * intention.peek().getUnif(); Term rec = send.getTerm(0).capply(un); if
     * (rec.isList()) { // send to many receivers // put the answers in the
     * unifier VarTerm answers = new VarTerm("AnsList___" + m.getInReplyTo());
     * ListTerm listOfAnswers = (ListTerm) un.get(answers); if (listOfAnswers ==
     * null) { listOfAnswers = new ListTermImpl(); un.unifies(answers,
     * listOfAnswers); } listOfAnswers.append(content); int nbReceivers =
     * ((ListTerm) send.getTerm(0)).size(); if (listOfAnswers.size() ==
     * nbReceivers) { // all agents // have answered
     * resumeSyncAskIntention(m.getInReplyTo(), send.getTerm(3), listOfAnswers);
     * } } else { resumeSyncAskIntention(m.getInReplyTo(), send.getTerm(3),
     * content); }
     *
     * // the message is not an ask answer } else if (conf.ag.socAcc(m)) {
     *
     * if (!m.isReplyToSyncAsk()) { // ignore answer after the timeout //
     * generate an event String sender = m.getSender(); if
     * (sender.equals(getUserAgArch().getAgName())) sender = "self";
     *
     * boolean added = false; if (!setts.isSync() &&
     * !ag.getPL().hasUserKqmlReceivedPlans() && content.isLiteral() &&
     * !content.isList()) { // optimisation // to // jump // kqmlPlans if
     * (m.getIlForce().equals("achieve")) { content =
     * add_nested_source.addAnnotToList(content, new Atom(sender));
     * C.addEvent(new Event(new Trigger(TEOperator.add, TEType.achieve,
     * (Literal) content), Intention.EmptyInt)); added = true; } else if
     * (m.getIlForce().equals("tell")) { content =
     * add_nested_source.addAnnotToList(content, new Atom(sender));
     * getAg().addBel((Literal) content); added = true; } }
     *
     * if (!added) { Literal received = new LiteralImpl(
     * Message.kqmlReceivedFunctor).addTerms(new Atom( sender), new
     * Atom(m.getIlForce()), content, new Atom(m.getMsgId()));
     *
     * updateEvents(new Event(new Trigger(TEOperator.add, TEType.achieve,
     * received), Intention.EmptyInt)); } } else {
     * logger.fine("Ignoring message " + m +
     * " because it is received after the timeout."); } } } }
     */
    ...
    private void applyProcMsg() throws JasonException {
        ...
        if (conf.C.hasMsg()) {
            ...
            if (intention != null) {
                ...
                } else if (content.isLiteral()) { // adds source in the content
                    // if possible
                    content = ((Literal) content).forceFullLiteralImpl();
                    ((Literal) content).addSource(new Atom(m.getSender()));
                } else if (send.getTerm(1).toString().equals("askAll") && content
                        .isList()) { // adds source in each answer if
                    // possible
                ...
                if (rec.isList()) { // send to many receivers
                    ...
                    if (listOfAnswers.size() == nbReceivers) { // all agents
                        // have answered
                        resumeSyncAskIntention(m.getInReplyTo(), send.getTerm(3), listOfAnswers);
                    }
                } else {
                ...
                if (!m.isReplyToSyncAsk()) { // ignore answer after the timeout
                    ...
                    boolean added = false;
                    if (!setts.isSync() && !ag.getPL().hasUserKqmlReceivedPlans() && content.isLiteral() && !content
                            .isList()) { // optimisation
                        // to
                        // jump
                        // kqmlPlans
    ...
    private void resumeSyncAskIntention(String msgId, Term answerVar, Term answerValue) throws JasonException {
        ...
        if (i.peek().getUnif().unifies(answerVar, answerValue)) {
            ...
            generateGoalDeletion(i, JasonException.createBasicErrorAnnots("ask_failed",
                    "reply of an ask message ('" + answerValue + "') does not unify with fourth argument of .send ('"
                            + answerVar + "')"));
        }
    ...
    private void applySelEv() throws JasonException {
        ...
        if (C.hasAtomicIntention()) {
            confP.step = State.ProcAct; // need to go to ProcAct to see if an
            // atomic intention received a feedback
            // action
            // código do márcio
            // test{
            // eventTimer.end();
            // if (eventTimer.list.size() > cicleNumber) {
            // System.out.println("Printing TS!");
            // eventTimer.storeEventInfo();
            // }
            // }
            return;
        }
        ...
        if (conf.C.hasEvent()) {
            ...
            if (confP.C.SE != null) {
                if (ag.hasCustomSelectOption() || setts.verbose() == 2) // verbose
                // == 2
                // means
                // debug
                // mode
    ...
    private void applyApplPl() throws JasonException {
        ...
        /*
         * test{ intentionTimer.end(); if (intentionTimer.list.size() > 20) {
         * //System.out.println("Printing TS!");
         * intentionTimer.storeEventInfo(); } //}
         */
    }
    ...
    private void applySelAppl() throws JasonException {
        ...
        if (confP.C.SO != null) {
            ...
            generateGoalDeletionFromEvent(
                    JasonException.createBasicErrorAnnots("no_option", "selectOption returned null"));
    ...
    /**
     * This step is new in Jason 1.1 and replaces the steps
     * RelPl->ApplPl->SelAppl when the user does not customise selectOption.
     * This version does not create the RP and AP lists and thus optimise the
     * reasoning cycle. It searches for the first option and automatically
     * selects it.
     *
     * @since 1.1
     */
            ...
            for (Plan pl : candidateRPs) {
                ...
                if (relUn != null) { // is relevant
                    ...
                    if (context == null) { // context is true
                        ...
                        // test{
                        // intentionTimer.end();
                        // if (intentionTimer.list.size() > cicleNumber) {
                        // System.out.println("Printing TS!");
                        // intentionTimer.storeIntentionInfo();
                        // }
                        // }
                        return;
                    } else {
    ...
    private void applyAddIM() throws JasonException {
        ...
        if (conf.C.SE.intention == Intention.EmptyInt) {
            ...
            if (setts.isTROon()) {
                IntendedMeans top = confP.C.SE.intention.peek(); // top = the IM
                // that will
                // be
                // removed
                // from the
                ...
                if (top != null && top.getTrigger().isGoal() && im.getTrigger().isGoal() && // are both goal
                        ...
                        top.getTrigger().getPredicateIndicator().equals(
                                im.getTrigger().getPredicateIndicator()) // goals
                    // are
                    // equals
                ) {
                    confP.C.SE.intention.pop(); // remove the top IM

                    IntendedMeans imBase = confP.C.SE.intention.peek(); // base
                    // =
                    // where
                    // the
                    // new
                    ...
                    // of
                    if (imBase != null) {
                        // move top relevant values into the base (relevant =
                        // renamed vars in base)
                        for (VarTerm v : imBase.renamedVars) {
                            VarTerm vvl = (VarTerm) imBase.renamedVars.function.get(v);
                            Term t = top.unif.get(vvl);
                            if (t != null) { // if v has got a value in top
                                // unif, put the value in the
                                // unifier
                                ...
                                // the vvl was renamed again in top, just
                                // replace in base the new value
    ...
    private void applyProcAct() throws JasonException {
        ...
        if (conf.C.hasFeedbackAction()) {
            ...
            if (a != null) {
                ...
                // remove the intention from PA (PA has all pending action,
                // including those in FA;
                // but, if the intention is not in PA, it means that the
                    ...
                    if (a.getResult()) {
                        ...
                        if (hasGoalListener()) {
                            for (GoalListener gl : getGoalListeners()) {
                                for (IntendedMeans im : confP.C.SI)
                                // .getIMs())
                                {
                                    gl.goalResumed(im.getTrigger());
                                }
                        ...
                        C.removeAtomicIntention(); // if (potential) atomic
                        // intention is not removed,
                        // it will be selected in
                        // selInt or selEv and runs
                        // again
                    }
                } else {
    ...
    private void applySelInt() throws JasonException {
        ...
        // Rule SelInt1
        if (!conf.C.isAtomicIntentionSuspended() && conf.C.hasIntention()) { // the
            ...
            if (logger.isLoggable(Level.FINE)) {
                logger.fine("Selected intention " + confP.C.SI);
            }
            if (confP.C.SI != null) { // the selectIntention function returned
                // null
                return;
            }
        }
    ...
    private void applyExecInt() throws JasonException {
        ...
        if (bTerm instanceof VarTerm) { // de-var bTerm
            ...
            if (bTerm.isVar()) { // the case of !A with A not ground
                ...
                if (!generateGoalDeletion(conf.C.SI,
                        JasonException.createBasicErrorAnnots("body_var_without_value", msg))) {
                    logger.log(Level.SEVERE, msg);
                }
                return;
            }
            if (bTerm.isPlanBody()) {
                if (h.getBodyType() != BodyType.action) { // the case of ...; A
                    // = { !g }; +g;
                    // ....
                    String msg = h.getSrcInfo() + ": " + "The operator '" + h.getBodyType()
                            + "' is lost with the variable '" + bTerm + "' unified with a plan body. ";
                    if (!generateGoalDeletion(conf.C.SI,
                            JasonException.createBasicErrorAnnots("body_var_with_op", msg))) {
                        logger.log(Level.SEVERE, msg);
                    }
                    return;
                }
        ...
        switch (h.getBodyType()) {
                ...
                try {
                    ...
                    if (oresult != null) {
                        ...
                        if (!ok && oresult instanceof Iterator) { // ia result is an
                            ...
                            if (iu.hasNext()) {
                                // change the unifier of the current IM to the first
                                // returned by the IA
                                im.unif = iu.next();
                                ok = true;
                            }
            ...
            case constraint:
                ...
                if (iu.hasNext()) {
                    ...
                    generateGoalDeletion(conf.C.SI,
                            JasonException.createBasicErrorAnnots(new Atom("constraint_failed"), msg));
                    logger.fine(msg);
                }
                break;
            ...
            case test:
                ...
                if (conf.ag.believes(f, u)) {
                    ...
                    boolean fail = true;
                    // generate event when using literal in the test (no events for
                    // log. expr. like ?(a & b))
                    if (f.isLiteral() && !(f instanceof BinaryStructure)) {
                        body = prepareBodyForEvent(body, u, conf.C.SI.peek());
                        if (body.isLiteral()) { // in case body is a var with
                            // content that is not a literal
                            // (note the VarTerm pass in the
                            // instanceof Literal)
                            Trigger te = new Trigger(TEOperator.add, TEType.test, body);
                            evt = new Event(te, conf.C.SI);
                            if (ag.getPL().hasCandidatePlan(te)) {
                                if (logger.isLoggable(Level.FINE)) {
                                    logger.fine("Test Goal '" + h
                                            + "' failed as simple query. Generating internal event for it: " + te);
                                }
                                conf.C.addEvent(evt);
                                confP.step = State.StartRC;
                                fail = false;
                    ...
                    if (fail) {
                        if (logger.isLoggable(Level.FINE)) {
                            logger.fine("Test '" + h + "' failed (" + h.getSrcInfo() + ").");
                        }
                        generateGoalDeletion(conf.C.SI, JasonException
                                .createBasicErrorAnnots("test_goal_failed", "Failed to test '" + h + "'"));
                    }
                }
                break;
            ...
            case delAddBel:
                ...
                b2.makeTermsAnnon(); // do not change body (but b2), to not
                // interfere in addBel
                ...
                try {
                    List<Literal>[] result = ag.brf(null, b2, conf.C.SI); // the
                    // intention
                    // is
                    // not
                    // the
                    ...
                    generateGoalDeletion(conf.C.SI, JasonException
                            .createBasicErrorAnnots("belief_revision_failed", "BRF failed for '" + body + "'"));
                    break;
                }
            ...
            case addBelNewFocus:
                ...
                try {
                    ...
                    if (h.getBodyType() == BodyType.addBelEnd) {
                        ...
                        result = ag.brf(body, null, conf.C.SI); // use default (well
                    }
                    // documented and
                    // used) method in
                    // case someone has
            ...
            case delBel:
                ...
                // call BRF
                try {
                    List<Literal>[] result = ag.brf(null, body, conf.C.SI); // the
                    // intention
                    // is
                    // not
                    // the
    ...
    private Literal prepareBodyForEvent(Literal body, Unifier u, IntendedMeans imRenamedVars) {
        ...
        body.makeVarsAnnon(renamedVars); // free variables in an event cannot
        // conflict with those in the plan
        if (imRenamedVars != null) {
            imRenamedVars.renamedVars = renamedVars;

            // Code for TRO (Tail Recursion Opt)
            if (setts.isTROon()) {
                // renamed vars binded with another var in u need to be
                // preserved (since u will be lost in TRO)
                    ...
                    if (t != null) {
                        ...
                        try {
                            ...
                            logger.log(Level.SEVERE,
                                    "*** Error adding var into renamed vars. var=" + v + ", value=" + t + ".", e);
                        }
                    }
                }
                if (adds != null) {
        ...
        if (!body.hasSource()) { // do not add source(self) in case the
    ...
    public void applyClrInt(Intention i) throws JasonException {
        while (true) { // quit the method by return
            ...
            if (logger.isLoggable(Level.FINE)) {
                logger.fine("Returning from IM " + topIM.getPlan().getLabel() + ", te=" + topIM.getPlan().getTrigger()
                        + " unif=" + topIM.unif);
            }
            ...
            if (im.getTrigger().isGoal() && !im.getTrigger().isAddition() && !i.isFinished()) {// i.size() > 0) {
                ...
                if (im.isFinished() || !(im.unif.unifies(im.getCurrentStep().getBodyTerm(), topLiteral)
                        && im.getCurrentStep().getBodyType() == BodyType.achieve) || im.getCurrentStep()
                        .getBodyTerm() instanceof VarTerm) {
                    im = i.pop(); // +!c above
                }
                while (!i.isFinished() && !(im.unif.unifies(im.getTrigger().getLiteral(), topLiteral) && im.getTrigger()
                        .isGoal()) && !(im.unif.unifies(im.getCurrentStep().getBodyTerm(), topLiteral)
                        && im.getCurrentStep().getBodyType() == BodyType.achieve)) {
                    im = i.pop();
                }
            }
            ...
            if (!i.isFinished()) {
                ...
                if (!im.isFinished()) {
                    ...
                    // old code:
                    /*
                     * topLiteral.apply(topIM.unif); topLiteral.makeVarsAnnon();
                     * Literal cstep = (Literal)im.removeCurrentStep(); boolean
                     * r = im.unif.unifies(cstep,topLiteral);
                     */

                    // new code optimised: handle directly renamed vars for the
                    // call
                    // get vars in the unifier that comes from makeVarAnnon
                    // (stored in renamedVars)
    ...
    private void joinRenamedVarsIntoIntentionUnifier(IntendedMeans im, Unifier values) {
        if (im.renamedVars != null) {
            for (VarTerm ov : im.renamedVars.function.keySet()) {
                ...
                im.unif.unifiesNoUndo(ov, vt); // introduces the renaming in the
                // current unif
                // if vt has got a value from the top (a "return" value),
                // include this value in the current unif
                ...
                if (context == null) { // context is true
                    ...
                    if (r != null) {
                        while (r.hasNext()) {
                            ...
                            if (!allUnifs) {
                                break; // returns only the first unification
                            }
                            if (r.hasNext()) {
    ...
    public void updateEvents(Event e) {
        ...
        // a) allow the user to override selectOption and then provide an
        // "unknown" plan; or then
        // b) create the failure event (it is done by SelRelPlan)
        if (e.isInternal() || C.hasListener() || ag.getPL().hasCandidatePlan(e.trigger)) {
            C.addEvent(e);
            if (logger.isLoggable(Level.FINE)) {
                logger.fine("Added event " + e + ", events = " + C.getEvents());
            }
        }
    }
    ...
    public boolean generateGoalDeletion(Intention i, List<Term> failAnnots) throws JasonException {
        ...
        if (im.isGoalAdd()) {
            ...
            if (failEventIsRelevant) {
                ...
                if (logger.isLoggable(Level.FINE)) {
                    logger.fine(
                            "Generating goal deletion " + failEvent.getTrigger() + " from goal: " + im.getTrigger());
                }
            } else {
    ...
    private boolean generateGoalDeletionFromEvent(List<Term> failAnnots) throws JasonException {
        ...
        if (tevent.isAddition() && tevent.isGoal()) {
            ...
            if (failEvent != null) {
                ...
                // logger.warning("Generating goal deletion " +
                // failEvent.getTrigger() + " from event: " + ev.getTrigger());
            } else {
    ...
    public Event findEventForFailure(Intention i, Trigger tevent) {
        ...
        if (i != Intention.EmptyInt) {
            ...
            while (!getAg().getPL().hasCandidatePlan(failTrigger) && ii.hasNext()) {
                // TODO: pop IM until +!g or *!g (this TODO is valid only if
                // meta events are pushed on top of the intention)
                // If *!g is found first, no failure event
                // - while popping, if some meta event (* > !) is in the stack,
                // stop and simple pop instead of producing an failure event
    ...
    /*
     * check if the event is a goal addition with hard deadline, if so, schedule
     * a verification after the deadline
     */
        ...
        getAg().getScheduler().schedule(new Runnable() {
            public void run() {
                runAtBeginOfNextCycle(new Runnable() {
                    public void run() {
                        boolean drop = false;
                        if (intention == null) { // deadline in !!g, test if the
                            // agent still desires it
    ...
    public boolean canSleep() {
        return (C.isAtomicIntentionSuspended() && !conf.C.hasMsg()) || (!conf.C.hasEvent() && !conf.C.hasIntention()
                && !conf.C.hasFeedbackAction() && !conf.C.hasMsg() &&
    ...
    public boolean reasoningCycle() {
        if (logger.isLoggable(Level.FINE)) {
            logger.fine("Start new reasoning cycle");
        }
        getUserAgArch().reasoningCycleStarting();

        /*
         * used to find bugs (ignore) int is = C.getIntentions().size(); int es
         * = C.getEvents().size(); if (is+es != 4) {
         * logger.info("1****"+is+" "+es); logger.info(C.toString()); try {
         * logger.info("======"+applyClrInt(C.SI));
         * logger.info("======"+C.SI.isFinished());
         * logger.info("======"+C.SI.getB()); //logger.info(""+getAg().getPL());
         * } catch (JasonException e) { e.printStackTrace(); }
         * logger.info("old 1"+pc1.toString());
         * logger.info("old 2"+pc2.toString());
         * logger.info("old 3"+pc3.toString());
         * logger.info("old 4"+pc4.toString()); System.exit(0); return false; }
         *
         * pc4 = pc3; pc3 = pc2; pc2 = pc1; pc1 = C.clone();
         */
        ...
        try {
            ...
            if (nrcslbr >= setts.nrcbp()) {
                ...
                if (this.agArch.getCommBridge() != null) {
                    //System.out.println("[ARGO][" + this.agArch.getAgName() + "]: Entering the first IF!");
                    if (this.agArch.getCommBridge().hasMsg()) {
                        this.agArch.addMessageToC();
                        this.agArch.getCommBridge().cleanMailBox();
                        //System.out.println("Nome Argo: "+ this.agArch.getNameArgo());
                    }

                    if (this.agArch.getCommBridge().hasAgentsReceived()) {
                        // Tenho agentes para instanciar.
                        this.agArch.instantiateAgents();
                    }
            ...
            if (canSleep()) {
                ...
                if (C.hasMsg()) { // the agent has messages
                    ...
                    // check if there is an event in C.E not produced by idle
                    // intention
                    for (Event e : C.getEvents()) {
                        Intention i = e.getIntention();
                        if (!e.getTrigger().equals(PlanLibrary.TE_JAG_SLEEPING) || (i != null && i.hasTrigger(
                                PlanLibrary.TE_JAG_SLEEPING, new Unifier()))) {
                            sleepingEvt = false;
                            break;
                        }
                    }
            ...
            if (action != null) {
                /*
                 * test{ timer.end(); if (timer.list.size() > 10) {
                 * //System.out.println("Printing TS!");
                 * timer.storeInternalInfo(); } //}
                 */
                C.addPendingAction(action);
                // We need to send a wrapper for FA to the user so that add
                // method then calls C.addFA (which control atomic things)
    ...
    public String toString() {
        return "TS of agent.asl " + getUserAgArch().getAgName();
    }

    public enum State {
        StartRC, SelEv, RelPl, ApplPl, SelAppl, FindOp, AddIM, ProcAct, SelInt, ExecInt, ClrInt
    }
    ...
    class FailWithDeadline extends fail_goal {
         ...
         *
         */
        private static final long serialVersionUID = 8250158277526232058L;
        ...
        /*
         * returns: >0 the intention was changed 1 = intention must continue
         * running 2 = fail event was generated and added in C.E 3 = simply
         * removed without event
         */
        @Override
        public int dropIntention(Intention i, Trigger g, TransitionSystem ts, Unifier un) throws JasonException {
            if (i != null) {
                // only consider dropping if the intention is the one that
                // created the deadline goal
                if (intToDrop == null) {
                    if (te != i.getBottom().getTrigger()) { // no intention,
                        // then consider the
                        // bottom trigger
                        return 0;
                    }
                ...
                if (i.dropGoal(g, un)) {
                    ...
                    Event failEvent = ts.findEventForFailure(i, g); // find fail
                    // event for
                    // the goal
                    // just
                    // dropped
                    if (failEvent != null) {
                        failEvent.getTrigger().getLiteral().addAnnots(
                                JasonException.createBasicErrorAnnots("deadline_reached", ""));
                        ts.getC().addEvent(failEvent);
                        ts.getLogger().fine(
                                "'hard_deadline(" + g + ")' is generating a goal deletion event: " + failEvent
                                        .getTrigger());
                        return 2;
                    } else { // i is finished or without failure plan
                        ts.getLogger().fine(
                                "'hard_deadline(" + g + ")' is removing the intention without event:\n" + i);
                        return 3;
                    }
                }
            }
