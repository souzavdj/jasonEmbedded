...
public class Circumstance implements Serializable {
    ...
    private Map<String, Intention> PI;
            // pending intentions, intentions suspended by any other reason


    private Map<String, Event> PE;
            // pending events, events suspended by .suspend
    ...
    public void insertMetaEvent(Event ev) {
        ...
        List<Event> newE = new ArrayList<Event>(
                E); // make a list of events to find the best place to insert the new event
        int pos = 0;
        for (Event e : newE) {
    ...
    public Intention removeAtomicIntention() {
        if (AI != null) {
            if (atomicIntSuspended) {
                //throw new JasonException("Internal error: trying to remove the atomic intention, but it is
                // suspended! it should be removed only when back to I!");
                return null;
            }
            Intention tmp = AI;
            removeIntention(AI);
