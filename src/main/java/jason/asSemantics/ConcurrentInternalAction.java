 ...
 *
 * startInternalAction(ts, new Runnable() { // to not block the agent thread, start a thread that performs the task
 * and resume the intention latter
 * public void run() {
 *
 * .... the code of the IA .....
 *
 ...
 * }
 *
 * public void timeout(TransitionSystem ts, String intentionKey) { // called back when the intention should be
 * resumed/failed by timeout (after 5 seconds in this example)
...
public abstract class ConcurrentInternalAction implements InternalAction {
    ...
    public void failInt(TransitionSystem ts, String intentionKey) {
        resume(ts, intentionKey, true,
                JasonException.createBasicErrorAnnots("fail_resume", "Error resuming pending intention"));
    }

    synchronized public static void resume(final TransitionSystem ts, final String intentionKey, final boolean abort,
                                           final List<Term> failAnnots) {
                ...
                if (pi != null) {
                    ...
                    try {
                        if (abort) {
                            ...
                            pi.peek()
                                    .removeCurrentStep(); // remove the internal action that put the intention in
                            // suspend
                            ts.applyClrInt(pi);
