...
public class Agent {
    ...
    /** Objetivos iniciais. */
    private List<Literal> initialGoals = null; // initial goals in the source

    /** Crenças iniciais. */
    private List<Literal> initialBels = null; // initial beliefs in the source
     ...
     * base for the agent.
     */
    public static Agent create(AgArch arch, String agClass, ClassParameters bbPars, String asSrc, Settings stts)
    throws JasonException {
        try {
            Agent ag = (Agent) Class.forName(agClass).newInstance();

            new TransitionSystem(ag, null, stts, arch); // [Fabian] O TS também vai para o agente.
    ...
    public HashMap<String, Filter> loadXMLBeliefFilter(String file) {
            ...
            for (int temp = 0; temp < nList.getLength(); temp++) {
                ...
                if (nNode.getNodeType() == Node.ELEMENT_NODE) {
                    ...
                    for (int i = 0; i < eElement.getElementsByTagName("parameter").getLength(); i++) {
                        ...
                        try {
                            p.setIndex(Integer.parseInt(eElement.getElementsByTagName("parameter").item(i)
                                    .getAttributes().getNamedItem("id").getTextContent()));
                            p.setOperator(getOperator(eElement.getElementsByTagName("parameter").item(i).getAttributes()
                                    .getNamedItem("operator").getTextContent()));
                            ...
                            try {
                                ...
                                for (int i = 0; i < nl.getLength(); i++) {
                                    ...
                                    if (nodeAnot.getChildNodes().item(i).getNodeName().equals("limit")) {
                                        ...
                                        anot.setOperator(getOperator(nodeAnot.getChildNodes().item(i).getAttributes()
                                                .getNamedItem("operator").getTextContent()));
                                    }
                                }
    ...
    private static Operator getOperator(String op) {
        if (op.toLowerCase().matches("be")) {
            return Operator.BE;
        }
        if (op.toLowerCase().matches("b")) {
            return Operator.B;
        }
        if (op.toLowerCase().matches("se")) {
            return Operator.SE;
        }
        if (op.toLowerCase().matches("s")) {
            return Operator.S;
        }
        return Operator.E;
    }
    ...
    public void load(String asSrc) throws JasonException {
        ...
        try {
            ...
            if (parsingOk) {
                ...
                addInitialGoalsInTS();
                fixAgInIAandFunctions(this); // used to fix agent reference in
    ...
    public TransitionSystem initAg(AgArch arch, BeliefBase bb, String asSrc, Settings stts) throws JasonException {
        try {
            ...
            logger.log(Level.SEVERE, "Error creating the agent.asl class!", e);
            throw new JasonException("Error creating the agent.asl class! - " + e);
        }
    }
    ...
    public Agent clone(AgArch arch) {
        ...
        a.initAg(); // for initDefaultFunctions() and for overridden/custom
        // agent
        return a;
    }

    private void fixAgInIAandFunctions(Agent a) throws Exception {
        // find all internal actions and functions and change the pointer for
        // agent
        for (Plan p : a.getPL()) {
            // search context
    ...
    private void fixAgInIAandFunctions(Agent a, Literal l) throws Exception {
        ...
        if (l instanceof InternalActionLiteral) {
            ((InternalActionLiteral) l).setIA(null); // reset the IA in the
            // literal, the IA there
    ...
    throws ClassNotFoundException, InstantiationException, IllegalAccessException {
        ...
        if (objIA == null) {
            @SuppressWarnings("rawtypes") Class iaclass = Class.forName(iaName);
            try {
                // check if the class has "create" method -- singleton
                // implementation
    ...
    public void addInitialBelsInBB() throws RevisionFailedException {
        // Once beliefs are stored in a Stack in the BB, insert them in inverse
        // order
        for (int i = initialBels.size() - 1; i >= 0; i--) {
            Literal b = initialBels.get(i);

            // if l is not a rule and has free vars (like l(X)), convert it into
            // a rule like "l(X) :- true."
            ...
            // does not do BRF for rules (and so do not produce events +bel for
            // rules)
            if (b.isRule()) {
                getBB().add(b);
            } else {
    ...
    private static boolean remove(Literal literal) {
        ...
        for (Parameter param : filter.getParameters()) {
            if (!compare(param.getValue(), getParameters(literal.toString()).length > param.getIndex() ?
                                           getParameters(literal.toString())[param.getIndex()] : null,
                    param.getOperator())) {
                return false;
            }
        }
        for (Annotation anot : filter.getAnotations()) {
            if (!compare(anot.getValue(), literal.getAnnots(anot.getPredicate()).size() != 0 ? literal.getAnnots(
                    anot.getPredicate()).toString().split("\\(")[1].split("\\)")[0] : null, anot.getOperator())) {
                return false;
            }
        }
        return true;
    ...
    public void buf(List<Literal> percepts) {
        ...
        while (perceptsInBB.hasNext()) {
            ...
            while (ip.hasNext()) {
                ...
                if (l.equalsAsStructure(t) && l.negated() == t.negated()) {
                    ...
                    // [can not be always removed, since annots in this percepts
                    // should be added in BB
                    // Jason team for AC, for example, use annots in
                    // perceptions]
                    if (!l.hasAnnot()) {
                        ip.remove();
                    }
            ...
            if (!wasPerceived) {
                dels++;
                // new version (it is sure that l is in BB, only clone l when
                ...
                /*
                 * // old version // can not delete l, but l[source(percept)] l
                 * = (Literal)l.clone(); l.clearAnnots();
                 * l.addAnnot(BeliefBase.TPercept); if (bb.remove(l)) {
                 * ts.updateEvents(new Event(new Trigger(TEOperator.del,
                 * TEType.belief, l), Intention.EmptyInt)); }
                 */
            }
        }
        // test
        ...
        if (qProfiling != null) {
            qProfiling.newUpdateCycle(getTS().getUserAgArch().getCycleNumber(), adds + dels,
                    System.nanoTime() - startTime);
        }

        //*** ativa o log *** C�digo do M�rcio ***
		/*
		timer.end(step);
    ...
    throws RevisionFailedException {
        ...
        try {
            ...
            if (beliefToDel != null) {
                ...
                if (!removed && !beliefToDel.isGround()) { // then try to unify
                    // the parameter
                    // with a belief in
                    // BB
    ...
    public void abolish(Literal bel, Unifier un) throws RevisionFailedException {
        ...
        if (il != null) {
            while (il.hasNext()) {
                ...
                if (!inBB.isRule()) {
                    // need to clone unifier since it is changed in previous
                    // iteration
    ...
    public Document getAgState() {
        ...
        document.appendChild(document.createProcessingInstruction("xml-stylesheet",
                "href='http://jason.sf.net/xml/agInspection.xsl' type='text/xsl' "));
...
class Timer {
    ...
    //fun��o para gravar o log de eventos. Feito pelo M�rcio.
	/*
	void storeEventInfo() {
		PrintWriter out = null;
		try {
			out = new PrintWriter(new BufferedWriter(new FileWriter(
					"test/event log.txt", true)));
			out.println(list.size());
			for (TimerStamp ts : list) {
				long elapsedTime = ts.end - ts.beginning;
				out.println("step " + ts.step + ": Time elapsed: "
						+ elapsedTime + " Begin:" + ts.beginning + " End:"
						+ ts.end);
				// step++;
			}
	...
	*/
    ...
    //fun��o para gravar o log de inten��es. Feito pelo M�rcio.
	/*
	void storeIntentionInfo() {
		PrintWriter out = null;
		try {
			out = new PrintWriter(new BufferedWriter(new FileWriter(
					"test/internal log.txt", true)));
			out.println(list.size());
			for (TimerStamp ts : list) {
				long elapsedTime = ts.end - ts.beginning;
				out.println("step " + ts.step + ": Time elapsed: "
	...
	*/
    ...
    //fun��o para gravar o log o BUF. Feito pelo M�rcio.
	/*
	void storeBufInfo() {
		PrintWriter out = null;
		try {
			out = new PrintWriter(new BufferedWriter(new FileWriter(
					"test/buf log.txt", true)));
			out.println(list.size());
			for (TimerStamp ts : list) {
				long elapsedTime = ts.end - ts.beginning;
				long elapsedFilter = ts.endFilter - ts.begFilter;
				out.println("step " + step + ":" + elapsedTime
						+ " Simulator at: " + ts.simulatorStep + " Time: "
						+ elapsedTime + " Filter time: " + elapsedFilter
						+ " Begin:" + ts.beginning + " End:" + ts.end);
				step++;
			}
