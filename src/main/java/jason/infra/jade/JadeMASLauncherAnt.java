...
public class JadeMASLauncherAnt extends CentralisedMASLauncherAnt implements MASLauncherInfraTier {
    ...
    protected String replaceMarks(String script, boolean debug) {
        ...
        if (!Config.checkJar(jadeJar)) {
            System.err.println("The path to the jade.jar file (" + jadeJar
                    + ") was not correctly set. Go to menu Plugin->Options->Jason to configure the path.");
        }
        ...
        String startContainers = "";
         /*
          ...
          + "        <java classname=\"jason.infra.jade.RunJadeMAS\" failonerror=\"true\" fork=\"yes\"
          dir=\"${basedir}\" >\n"
        ...
        for (String container : containers) {
            ...
            if (container.equals("Main-Container")) {
                ...
                if (!JadeAgArch.isCartagoJadeCase(project)) {
                    agents.append(
                            RunJadeMAS.environmentName + ":" + JadeEnvironment.class.getName() + "(j-project," + project
                                    .getProjectFile().getName() + ")");
                    sep = ";";
                }
            ...
            for (AgentParameters ap : project.getAgents()) {
                for (int cAg = 0; cAg < ap.qty; cAg++) {
                    ...
                    if ((container.equals("Main-Container") && ap.getHost() == null && allocator == null) || (
                            ap.getHost() != null && ap.getHost().equals(container)) || (allocator != null
                            && allocator.allocateAgent(numberedAg) != null && allocator.allocateAgent(numberedAg)
                            .equals(container))) {
                        agents.append(sep + numberedAg + ":" + JadeAgArch.class.getName() + "(j-project," + project
                                .getProjectFile().getName() + "," + ap.getAgName() + ")");
                        sep = ";";
                    }
                }
            }
            startContainers += "    <target name=\"" + container + "\" depends=\"compile\" >\n"
