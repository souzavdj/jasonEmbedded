...
public class JadeAgArch extends JadeAg {
    ...
    */
        ...
        if (args[0] instanceof AgentParameters) {
            ...
            if (args.length != 3) {
                logger.log(Level.SEVERE,
                        "To start agents from .mas2j file, you have to provide as parameters: (j-project <file.mas2j>"
                                + " <nameofagent in mas2j>)");
                return null;
            }
            ...
            if (ap == null) {
                logger.log(Level.SEVERE, "There is no agent.asl '" + args[2] + "' in project '" + args[1] + "'.");
            } else {
    ...
    public static boolean isCartagoJadeCase(MAS2JProject project) {
        return project.getEnvClass() != null && project.getEnvClass().getClassName().equals(
                CartagoEnvironment.class.getName()) && project.isJade();
    }
    ...
    private void registerAgInDF() {
        ...
        try {
            ...
            try {
                ...
                logger.log(Level.SEVERE, "Error registering agent.asl in DF", e);
                logger.log(Level.SEVERE, "Error unregistering agent.asl in DF", ex);
            }
        }
    }
                ...
                if (processExecutionControlOntologyMsg()) {
                    ...
                    try {
                        ...
                        if (logger.isLoggable(Level.FINE)) {
                            logger.fine("Informing controller that I finished a reasoning cycle " + jasonBridgeAgArch
                                    .getCycleNumber() + ". Breakpoint is " + isBreakPoint);
                        }
    ...
    private MessageTemplate ts = MessageTemplate.and(MessageTemplate.MatchContent("agState"),
            MessageTemplate.MatchOntology(JadeExecutionControl.controllerOntology));

    private MessageTemplate tc = MessageTemplate.and(MessageTemplate.MatchContent("performCycle"),
            MessageTemplate.MatchOntology(JadeExecutionControl.controllerOntology));
