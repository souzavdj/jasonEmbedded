...
public class JadeExecutionControl extends JadeAg implements ExecutionControlInfraTier {
    ...
    public void setup() {
        ...
        try {
            ...
            if (args != null && args.length > 0) {
                if (args[0] instanceof ClassParameters) { // it is an mas2j parameter
                    ...
                    if (args.length > 1) {
                        logger.warning(
                                "Execution control arguments is not implemented yet (ask it to us if you need)!");
                    }
                }
            } else {
        ...
        try {
            ...
            logger.log(Level.SEVERE, "Error starting agent.asl", e);
        }
    }
        ...
        addBehaviour(new OneShotBehaviour() {
            public void action() {
                try {
                    ...
                    ACLMessage r = ask(m);
                    if (r == null) {
                        System.err.println("No agent.asl state received! (possibly timeout in ask)");
                    } else {
