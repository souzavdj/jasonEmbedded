...
public class CentralisedMASLauncherAnt implements MASLauncherInfraTier {
    ...
    public String[] getStartCommandArray() {
        ...
        if (hasCBuild()) {
            build = bindir + "c-build.xml";
        }

        return new String[]{Config.get().getJavaHome() + "bin" + File.separator + "java", "-classpath",
                Config.get().getAntLib() + "ant-launcher.jar", "org.apache.tools.ant.launch.Launcher", "-e", "-f",
                build, task};
    }
    ...
    protected String replaceMarks(String script, boolean debug) {
        ...
        // if cartago env
        if (project.isJade() || (project.getEnvClass() != null && project.getEnvClass().getClassName().equals(
