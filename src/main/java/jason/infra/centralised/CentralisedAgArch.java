...
public class CentralisedAgArch extends AgArch implements Runnable {
     ...
     * the TS.
     */
    public void createArchs(List<String> agArchClasses, String agClass, ClassParameters bbPars, String asSrc,
                            Settings stts, RunCentralisedMAS masRunner) throws JasonException {
        try {
            this.masRunner = masRunner;

            // [Fabian] Criando agentes.
            ...
            running = false;
            throw new JasonException("as2j: error creating the agent.asl class! - " + e.getMessage(), e);
        }
    }
    ...
    public void createArchs(List<String> agArchClasses, Agent ag, RunCentralisedMAS masRunner) throws JasonException {
        try {
            ...
            running = false;
            throw new JasonException("as2j: error creating the agent.asl class! - ", e);
        }
    }
        ...
        if (rec == null) {
            if (isRunning()) {
                throw new ReceiverNotFoundException(
                        "Receiver '" + m.getReceiver() + "' does not exists! Could not send " + m);
            } else {
                return;
            }
        }
