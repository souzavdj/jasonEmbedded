...
public class JasonException extends Exception {
    ...
    public static JasonException createWrongArgumentNb(DefaultInternalAction ia) {
        ...
        return new JasonException("The internal action '" + ia.getClass().getSimpleName()
                + "' has not received the required number of argument(s)." + msg, WRONG_ARGS);
    }

    public static JasonException createWrongArgument(InternalAction ia, String reason) {
        return new JasonException(
                "Wrong argument for internal action '" + ia.getClass().getSimpleName() + "': " + reason, WRONG_ARGS);
    }
    ...
    public static ListTerm createBasicErrorAnnots(Term id, String msg) {
        return ASSyntax.createList(ASSyntax.createStructure("error", id),
