 ...
 * @see my_name
 */
public class sendOutCN extends DefaultInternalAction {
        ...
        if ((m.isTell() || m.isUnTell() || !m.isKnownPerformative()) && args.length > 3) {
            ...
            if (!mid.isAtom()) {
                throw new JasonException(
                        "The Message ID ('" + mid + "') parameter of the internal action 'send' is not an atom!");
            }
    ...
    private void delegateSendToArch(Term to, TransitionSystem ts, Message m) throws Exception {
        ...
        m.setReceiver(toString);
        ts.getUserAgArch().getCommBridge().sendMsgToContextNet(toString, m);
