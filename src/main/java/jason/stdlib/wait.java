    ...
    class WaitEvent implements CircumstanceListener {
        ...
        void resume(final boolean stopByTimeout) {
            ...
            ts.runAtBeginOfNextCycle(new Runnable() {
                public void run() {
                    try {
                        // add SI again in C.I if (1) it was not removed (2) is is not running (by some other reason)
                        // -- but this test does not apply to atomic intentions --, and (3) this wait was not dropped
                        if (c.removePendingIntention(sEvt) == si && (si.isAtomic() || !c.hasIntention(si))
                                && !dropped) {
                            if (stopByTimeout && te != null && elapsedTimeTerm == null) {
                                ...
                                if (si.isSuspended()) { // if the intention was suspended by .suspend
                                    ...
                                    body.add(1, new PlanBodyImpl(BodyType.internalAction,
                                            new InternalActionLiteral(".fail")));
                                    c.addPendingIntention(suspend.SUSPENDED_INT + si.getId(), si);
                                } else {
                                    ts.generateGoalDeletion(si,
                                            JasonException.createBasicErrorAnnots("wait_timeout", "timeout in .wait"));
                                }
                            } else {
        ...
        public void eventAdded(Event e) {
            ...
            if (te != null && un.unifies(te, e.getTrigger())) {
                resume(false);
            } else if (formula != null && ts.getAg().believes(formula,
                    un)) { // each new event, just test the formula being waited
                resume(false);
            }
        }
