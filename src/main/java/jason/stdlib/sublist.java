 ...
 * <li> <code>.sublist(X,[a,b,c])</code>: unifies X with any sublist of the list, i.e., [a,b,c], [a,b], [a], [b,c],
 * [b], [c], and [] in this order;
 * note that this is not the order in which its usual implementation would return in logic programming (see note on
 * .prefix).</li>
 *
 * </ul>
 *
...
public class sublist extends DefaultInternalAction {
    ...
    public Object execute(TransitionSystem ts, final Unifier un, final Term[] args) throws Exception {
        ...
        return new Iterator<Unifier>() {
            ...
            List<Term> list = listOutter.getAsList();
                    // used in the inner loop, Java List is used for faster remove in the end
