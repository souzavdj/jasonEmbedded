...
public class MAS2JProject {
    ...
    private ClassParameters infrastructure = new ClassParameters("Javino");
    ...
    public void registerDirectives() {
        if (directiveClasses != null) {
            for (String id : directiveClasses.keySet()) {
                try {
                    DirectiveProcessor.addDirective(id,
                            (Directive) Class.forName(directiveClasses.get(id)).newInstance());
                } catch (Exception e) {
    ...
    public String toString() {
        ...
        if (getEnvClass() != null && !getEnvClass().getClassName().equals(
                jason.environment.Environment.class.getName())) {
        ...
        if (infraFac == null) {
            try {
                ...
                throw new JasonException(
                        "The project's infrastructure ('" + infrastructure.getClassName() + "') is unknown! \n" + e);
            }
        }
        return infraFac;
    }
