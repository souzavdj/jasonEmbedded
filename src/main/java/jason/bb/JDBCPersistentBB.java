...
public class JDBCPersistentBB extends ChainBBAdapter {
    ...
    public void init(Agent ag, String[] args) {
        ...
        try {
            ...
            logger.log(Level.SEVERE,
                    "DB connection failure. url= " + url + ", user=" + args[2] + ", password=" + args[3], e);
        }
        nextBB.init(ag, args);
    }
    ...
    protected boolean isCreatedByJason(PredicateIndicator pi) throws SQLException {
        ...
        if (meta != null) {
            ...
            return cols >= extraCols && meta.getColumnName((cols - extraCols) + 1).equalsIgnoreCase(COL_NEG) && meta
                    .getColumnName((cols - extraCols) + 2).equalsIgnoreCase(COL_ANNOT);
        }
        return false;
    }
            ...
            if (bl != null) {
                if (isCreatedByJason(l.getPredicateIndicator())) {
                    ...
                    // the current bel bl already has l's annots
                    {
                        ...
                        // store bl annots
                        stmt = conn.createStatement();
                        String q = "update " + getTableName(bl) + " set " + COL_ANNOT + " = '" + bl.getAnnots() + "' "
                                + getWhere(l);
                        if (logger.isLoggable(Level.FINE)) {
                            logger.fine("query for update " + q);
                        }
                ...
                stmt = conn.createStatement();
                if (logger.isLoggable(Level.FINE)) {
                    logger.fine("query for insert " + getInsert(l));
                }
            ...
            try {
                if (l.hasSubsetAnnot(bl)) {
                    ...
                    if (bl.hasAnnot() && isCreatedByJason(l.getPredicateIndicator())) {
                        // store new bl annots
                        stmt.executeUpdate(
                                "update " + getTableName(bl) + " set " + COL_ANNOT + " = '" + bl.getAnnots() + "' "
                                        + getWhere(l));
                    } else {
                        // remove from DB
    ...
    public static Structure timestamp2structure(Timestamp timestamp) throws SQLException {
        ...
        return ASSyntax.createStructure(timestampFunctor, createNumber(time.get(Calendar.YEAR)),
                createNumber(time.get(Calendar.MONTH)), createNumber(time.get(Calendar.DAY_OF_MONTH)),
                createNumber(time.get(Calendar.HOUR_OF_DAY)), createNumber(time.get(Calendar.MINUTE)),
    ...
    public static Timestamp structure2timestamp(Term timestamp) throws Exception {
        if (timestamp.isStructure()) {
            ...
            if (s.getFunctor().equals(timestampFunctor) && s.getArity() == 6) {
                return new Timestamp((int) ((NumberTerm) s.getTerm(0)).solve() - 1900, (int) ((NumberTerm) s.getTerm(1))
                        .solve(), (int) ((NumberTerm) s.getTerm(2)).solve(), (int) ((NumberTerm) s.getTerm(3)).solve(),
                        (int) ((NumberTerm) s.getTerm(4)).solve(), (int) ((NumberTerm) s.getTerm(5)).solve(), 0);
            }
        }
        return null;
    }
